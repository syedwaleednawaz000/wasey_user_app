...
class _OnHoverState extends State<OnHover> {
  ...
  Widget build(BuildContext context) {
    ...
    final shadow1 = BoxDecoration(
      ...
      color: widget.fromMenu ? Theme.of(context).primaryColor.withAlpha((0.10 * 255).toInt()) : Theme.of(context).cardColor,
      boxShadow: [
        widget.fromMenu ? const BoxShadow(color: Colors.transparent) :
        BoxShadow(
          color: Get.find<ThemeController>().darkTheme ? const Color(0xFFBDBDBD).withAlpha((0.1 * 255).toInt()) : Theme.of(context).primaryColor.withAlpha((0.5 * 255).toInt()),
          blurRadius: 10,
