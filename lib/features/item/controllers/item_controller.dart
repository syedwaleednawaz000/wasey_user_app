...
class ItemController extends GetxController implements GetxService {
  ...
  // void onInit() {
  //   super.onInit();
  //   // Initialize slice selections when controller is created
  //   ever(selectedVariations, (List<List<bool?>> variations) {
  //     _initializeDefaultSlices();
  //   });
  // }
  //
  // // Method to initialize default slice selections
  // void _initializeDefaultSlices() {
  //   if (_item != null && isPizzaItem(_item!)) {
  //     for (int vIndex = 0; vIndex < _item!.foodVariations!.length; vIndex++) {
  //       for (int oIndex = 0; oIndex < _item!.foodVariations![vIndex].variationValues!.length; oIndex++) {
  //         String key = '$vIndex-$oIndex';
  //         // Only set default to full slice if this option is selected
  //         if (vIndex < selectedVariations.length &&
  //             oIndex < selectedVariations[vIndex].length &&
  //             selectedVariations[vIndex][oIndex] == true) {
  //           sliceSelections[key] = 1; // Default to full slice for selected options
  //         }
  //       }
  //     }
  //   }
  ...
  int getSliceSelection(int variationIndex, int optionIndex) {
    ...
    return sliceSelections[key] ?? 1; // 0 means no selection
  }
