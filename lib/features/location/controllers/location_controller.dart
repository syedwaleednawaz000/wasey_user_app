...
class LocationController extends GetxController implements GetxService {
  ...
  Future<ZoneResponseModel> getZone(String? lat, String? lng, bool markerLoad,
    ...
    if (!updateInAddress) {
      update();
      log("address updated in local updateInAddress != true");
    }
    log("address updated in local updateInAddress == true");
  ...
  Future<ZoneResponseModel> getZoneForLogin(String? lat, String? lng, bool markerLoad,
      {bool updateInAddress = false, bool handleError = false}) async {
    if (markerLoad) {
      _loading = true;
    } else {
  ...
  void _saveDataAndFirebaseConfig(AddressModel address, bool fromSignUp,
    ...
    await AddressHelper.saveUserAddressInSharedPref(address);
    debugPrint('_saveDataAndFirebaseConfig: ===============');
  ...
  Future<void> navigateToLocationScreen(String page,
    ...
    debugPrint('navigateToLocationScreen: ===============');
    bool fromSignup = page == RouteHelper.signUp;
    bool fromHome = page == 'home';

    debugPrint('(!AuthHelper.isLoggedIn() && !AuthHelper.isGuestLoggedIn()): ${(!AuthHelper.isLoggedIn() && AuthHelper.isGuestLoggedIn())}  ${(!AuthHelper.isLoggedIn())} && ${AuthHelper.isGuestLoggedIn()}');
  ...
  void _checkPermission(String page) async {
    ...
    if (permission == LocationPermission.denied ||
      ...
      debugPrint('_checkPermission accessed: ===============');
      // ADD AUTH CHECK HERE - Redirect to sign-in instead of location flow
      if (!AuthHelper.isLoggedIn()) {
        Get.offAllNamed(RouteHelper.signIn);
        return;
      }
      if (page == 'home') {
        debugPrint('page == home: ===============');
        Get.toNamed(RouteHelper.getPickMapRoute(page, false));
