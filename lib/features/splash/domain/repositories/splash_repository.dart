...
class SplashRepository implements SplashRepositoryInterface {
  ...
  Future<ModuleModel?> initSharedData() async {
    ...
    ModuleModel? module;
    if(sharedPreferences.containsKey("moduleId")) {
      try {
        module = ModuleModel.fromJson(jsonDecode(sharedPreferences.getString("moduleId")!));
      }catch(e) {
  ...
  ModuleModel? getModule() {
    ModuleModel? module;
    if(sharedPreferences.containsKey('moduleId')) {
      log("Module id inside SplashRepo line 206: ${sharedPreferences.getString("moduleId")}");
      try {
        module = ModuleModel.fromJson(jsonDecode(sharedPreferences.getString("moduleId")!));
      }catch(e) {
        debugPrint('Did not get shared Preferences module. Note: $e');
