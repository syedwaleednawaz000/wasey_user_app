...
class _InterestScreenState extends State<InterestScreen> {
  ...
  void initState() {
    super.initState();
    log("Interest screen loaded...");
    _selectAllCategoriesAndSave();
    // Get.find<CategoryController>().getCategoryList(true, allCategory: false);
  }

  Future<void> _selectAllCategoriesAndSave() async {
    final categoryController = Get.find<CategoryController>();
    await categoryController.getCategoryList(true, allCategory: false);
    if (categoryController.categoryList != null &&
        categoryController.categoryList!.isNotEmpty) {
      List<int?> interests = [];
      for (var category in categoryController.categoryList!) {
        interests.add(category.id);
      }
      bool isSuccess = await categoryController.saveInterest(interests);
      if (isSuccess) {
        ...
        log("failed_to_save_interests");
        if (mounted) {
          Get.back();
        }
      }
    } else {
      log("no_category_found");
      if (mounted) {
        Get.back();
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      //                           style: STCMedium.copyWith(fontSize: 22),
      //                         ),
      //                         const SizedBox(
