...
// Add this:
  ...
  static List<GetPage> routes = [
    ...
    GetPage(name: storeDetails, page: () {
      // Retrieve the store object passed as arguments
      MarketStoreModel store = Get.arguments as MarketStoreModel;
      return StoreDetailsScreen(store: store);
    }),
    GetPage(
      name: market,
      page: () => const MarketScreen(),
      binding: MarketBinding(), // Associate the binding with the market screen route
    ),

    GetPage(
      name: storeDetails, // Already defined
      page: () {
        MarketStoreModel store = Get.arguments as MarketStoreModel;
        return StoreDetailsScreen(store: store);
      },
      // No specific binding needed here if StoreDetailsScreen is simple
      // and gets all its data via arguments. If it needs its own controller,
      // you'd add a StoreDetailsBinding.
    ),
  ];
